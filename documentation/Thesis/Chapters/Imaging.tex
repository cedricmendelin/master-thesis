...
Computed Tomography is a similar problem, but slightly simpler as problem can be in 2D, and is therefore well suited as a
...
\begin{figure}[h]
    ...
    % \centering
    \hfill
    \subbottom[Clean reconstruction: $\textit{FBP}(p, \theta, s)$  \label{fig:fbp_phantom}]{\includegraphics[width=0.24\textwidth]{fbp_phantom_clean.png}}
    \hfill
    \subbottom[Noisy reconstruction: $\textit{FBP}(y, \theta, s)$ \label{fig:fbp_phantom_noisy}]{\includegraphics[width=0.24\textwidth]{fbp_phantom_snr_10.png}}
    \hfill
    \subbottom[Noisy reconstruction: $\textit{UNet}(\textit{FBP}(y, \theta, s))$ \label{fig:fbp_unet_phantom_noisy}]{\includegraphics[width=0.24\textwidth]{fbp_unet_phantom_snr_10.png}}
    \hfill
...
Today's state-of-the-art reconstruction algorithms are Depp-Learning based.
...
Based on many observed micrographs, collected by the electron microscope, the original object $x$ will be estimated.
...
First, a similar notation as before is used, but in a more general way with
