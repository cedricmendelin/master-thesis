...
"""
def getRotationMatrix(angles,order='ZYX'):
    rr = scipy_rot.from_euler(order,angles,degrees=True)
    mat = rr.as_matrix()
    rotationMatrix = mat
    ...
    # # using "X1Y2Z3" convention
    # rotationMatrix = np.zeros((angles.shape[0],3,3))
    # if order=='XYZ':
    #     rotationMatrix[:,0,0] = c2*c3
    #     rotationMatrix[:,1,0] = c1*s3+c3*s1*s2
    #     rotationMatrix[:,2,0] = s1*s3-c1*c3*s2
...
def rotate_t(input_tensor, rotation_matrix):
    ...
    rotated_signal = F.grid_sample(input=input_tensor, grid=tmp, mode='bilinear',  align_corners=False).squeeze(0)
