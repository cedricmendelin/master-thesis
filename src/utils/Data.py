...
def create_or_load_vedo_images(dataset, name, N, image_size, SNR_value, save=True):
    image_filename = 'original_images_{0}_{1}_{2}.npz'.format(name, N, image_size)
    noisy_filename = 'noisy_images_{0}_{1}_{2}_{3}.npz'.format(name, N, image_size, SNR_value)
...
def create_or_load_knn_rotation_invariant(name, N, image_res, original_images, K, snr=None, save=True):
    ...
    if path.isfile(aspireknn_filename):
        ...
        classes, reflections, rotations, shifts, correlations = aspire_knn_with_rotation_invariant_distance(original_images, K)
        if save:
