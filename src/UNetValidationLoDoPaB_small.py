...
for i in tqdm(range(validation_count)):
  ...
  normalized_image = (t_validation_images[i] - torch.min(t_validation_images[i])) / (torch.max(t_validation_images[i]) - torch.min(t_validation_images[i]))
  normalized_reco = (reco_fbp_unet - torch.min(reco_fbp_unet)) / (torch.max(reco_fbp_unet) - torch.min(reco_fbp_unet))

  print(f"SNR {find_SNR(t_validation_images[i],  reco_fbp_unet)} -- SNR normalized {find_SNR(normalized_image, normalized_reco)}")

  plot_imshow(normalized_image.numpy())
  plot_imshow(normalized_reco.numpy())
