...
nu = 1  # or any other integer (10 leads to 1002 points, 4 to 162)
N_inplane = 10 # total with 4,8 : 1296
resolution = 50
K=3
recreate_data = True
debug = True
...
rng = default_rng()
rot_z = np.linspace(0,2*np.pi / N_inplane * (N_inplane-1), N_inplane)
print(rot_z)
...
print(f"original graph degree:{get_degree_matrix(graph)}")
...
######################### VDM ###############################
from utils.VectorDiffusionMaps import *
clean_graph = Knn(distance_per_node, classes, np.zeros_like(angle_per_node), None)

epsilon_vdm = 0.7
# verify_epsilon(clean_graph.distance, [20000, 40000, 60000, 80000, 90000])
# verify_epsilon(distances, [ 0.7 ])
...
print(f"vdm graph degree:{get_degree_matrix(vdm_graph)}")
