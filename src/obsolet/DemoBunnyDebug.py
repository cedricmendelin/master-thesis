...
plt.axis('off')
plt.savefig("GL_rot_inv_dist.pdf",bbox_inches='tight', pad_inches=0)
...
S = np.zeros((Ntheta//Nz*2,Ntheta//Nz*2))
Dinv = np.zeros((Ntheta//Nz*2,Ntheta//Nz*2))
for i in tqdm(range(Ntheta//Nz)):
    for j in range(i+1,Ntheta//Nz):
        theta = angles_est[i,j]/180*np.pi
...
aff = np.zeros((Ntheta//Nz,Ntheta//Nz))
for i in tqdm(range(Ntheta//Nz)):
    for j in range(i+1,Ntheta//Nz):
