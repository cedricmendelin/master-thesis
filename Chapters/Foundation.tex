...
The edges of the graph can be \textit{directed} or \textit{undirected}, which means that 
the node ordering in the graph matters, or not. In a directed graph, a edge points explicitly from
...
Moreover, edges can have weights, which is a method to define some more important neighbours to a node.
If edges are dealing with weights, we are talking from a \textit{weighted} graph.

A \textit{dense} graph is a graph, where the number of edges in close to the maximal number of edges.
...
This leads to an \textit{unweighted} graph, as the weight of all edges will be 1. 

When the graph is undirected, the resulting matrix will be symmetric and has therefore a complete set of eigenvalues
and eigenvectors. The set of eigenvalues are also called \textit{spectrum} of the graph.

\subparagraph{K-hop neighbourhood:}
The adjacency matrix $A$ has many a nice property, and one is referred to the k-hop neighbourhood. 
...
And a final normalization is the symmetric normalization, which preserves the nice symmetric of undirected graphs:
\begin{equation}
...
When data is not in form of a graph available, a graph from the data can be constructed.
...
as $\mathbb{N}_i$ and consists of the nodes, with the the $k$ smallest similarity measure.
...
to define graphs and result in a graph, where not all nodes have the same amount of edges.
...
$y_n = y + \eta$, where $\eta$ is the observation noise.
...
\subparagraph{subsection}{Connection to link prediction}
...
The radon transform then transform $f$ to a function $Rf$, which corresponds to the line integral of the line defined by 
...
which are also called sinogram, can be see in Figure~\ref{fig:phantom_sinogram}
